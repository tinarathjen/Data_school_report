...
flw_hd_data_all <-  flw_hd_data %>% 
  filter(obs_type=="half_fwr") %>% 
  group_by(genotype, reps) %>% 
  spread(key=environment,value=thermaltime) %>% 
  group_by(genotype) %>% 
  summarise(vern_effect=mean(LN)/mean(LV), pp_effect=mean(SV)/mean(LV)) 


flw_hd_data_all%>% 
  ggplot(mapping=aes(x=vern_effect, y=pp_effect))+
  geom_point()+
geom_text( aes(x=vern_effect, y=pp_effect, label=genotype, size=6,angle=45, alpha=0.5, colour="blue"))
```

```{rr standard-plot4, echo=FALSE, fig.align='center', fig.height=4, fig.width=6, out.width='100%'}

#Plots of correlation
plot1 <- ggplot(all_data_wide, mapping=aes(x=flw_tt, y=hd_tt))+
  geom_point(alpha=0.4, colour="grey49")+
  geom_smooth(method=lm, colour="blue")

plot2 <- ggplot(all_data_wide, mapping=aes(x=flw_tt, y=final_leaf))+
  geom_point(alpha=0.4, colour="grey49")+
  geom_smooth(method=lm, colour="blue")  

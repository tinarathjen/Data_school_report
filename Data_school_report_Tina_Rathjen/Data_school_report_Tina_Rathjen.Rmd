...
# My Project
I work on a project within the GRDC's National Phenology Initiative which aims to predict the flowering time of wheat and barley cultivars in all the different cerael growing regions within Australia.

Wheat and barley cultivars have an optimal flowering window. Crops that flower too early can have reduced yield due to exposure to insufficient biomass accummulation or to cold or frost events. Conversely, crops that flower too late risk being exposed to water stress or heat events. Growers require accurate information to select the correct cultivar and sowing date for their conditions. For new cultivars it many take several years to conduct field trials to acccumulate sufficient data to predict flowering time.  Also a cultivar is currently marketed as a spring or winter wheat and also as slow, mid or fast maturing but these terms can be misleading. With new cultivars being released yearly growers may not have the knowledge to decide which cultivar to plant and what is the correct time of sowing for their conditions.
...
The controlled experiments were carried out under four environmental conditions, SN (short days, no vernalisation), LN (long days, no vernalisation, SV (short days plus vernalisation) and LV (long days plus vernalisation) to determine the influence of photoperiod and vernalisation on flowering times. The temperature for all experiments was set at a constant 22C and measured every 30 minutes using a TinyTag data logger.  Traits measured include emergence date, flowering date, heading date, final leaf number and spikelet number. In addition haun stage, a measure of developmental growth stage based on leaf emergence, was recorded every third day from emergence to flowering. The data was initially in several sheets within one Excel file. I have used the tidyverse package to clean and arrange the data. I have separated the data into two files, Haun stage data, containing the haun scores and a second file Flowering data, containing all other traits.  I have also used the temperature data to convert dates to accumulated thermal time (degree-days). Tables 1 and 2 show a representation of the two tidied data files.


**Tables**

``````{r mytable, out.width='90%', echo = FALSE}
...
```{r secondtable, out.width='90%', echo = FALSE}
...
```{r standard-plot, echo=FALSE, fig.align='center', fig.height=4, fig.width=6, out.width='100%'}
...
```{r second_plot, echo=FALSE, fig.align='center', fig.height=4, fig.width=5, out.width='80%'}
...
```{r third_plot, echo=FALSE, fig.align='center', fig.height=4, fig.width=6, out.width='100%'}
...
```{r r fourth_plot, echo=FALSE, fig.align='center', fig.height=4, fig.width=6, out.width='100%'}
#all varieties
...
all_data_wide3%>% 
  ...
  geom_point()+
labs(title="Still to Fill in", x="Vernalisation effect", y="Photoperiod effect", size = 4)+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, size = 5),
        plot.title = (element_text(size=12, hjust = 0.5)),,
        panel.grid.minor.x  = element_blank())
...
```{r standard-plot4, echo=FALSE, fig.align='center', fig.height=4, fig.width=6, out.width='100%'}

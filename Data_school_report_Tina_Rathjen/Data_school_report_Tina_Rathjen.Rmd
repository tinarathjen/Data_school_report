...
Wheat cultivars have an optimal flowering window. Crops that flower too early can have lower yield due to insufficient biomass accummulation or exposure to cold or frost events. Conversely, crops that flower too late risk being exposed to water stress or heat events which can negatively impacts yield. Growers require accurate information to select the correct cultivar and sowing date for their farming conditions. For new cultivars it takes several years conducting field trials to accumulate data to predict flowering time. With new cultivars being released it is essential that growers have accurate information by the time of release.
...
# Next steps
It would also be interesting to examine the data further. One possibility is to see how the data looks when considering photo-thermal time (takes into account the different daylength) instead of just thermal-time.
...
I would love to learn more and find more uses for R. Hopefully this is only the start of the data journey.
